...
args.n_epoch = 5
args.subsample_size = 8192  # Aumentado para permitir mais pontos por amostra
args.batch_size = 2
...
def cloud_loader(tile_name, features_used, file_type='ply', max_points=8192, augment=False):
    ...
    desired_flange_points = max(int(max_points * 0.4), 1)  # Garantir pelo menos 20% de pontos de flanges
...
num_valid = int(len(pointcloud_train_files) * valid_ratio) + 2  # Adicionar 2 equipamentos para validação
...
class PointNet2SemSeg(nn.Module):
    def __init__(self, num_classes=2, input_channels=8):
        ...
        # Feature Propagation Layers - Ajustes nos canais de entrada
...
if __name__ == "__main__":
    ...
    if len(test_list) > 0:
        # Visualizar mais 2 equipamentos
